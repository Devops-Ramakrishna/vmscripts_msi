...
create_container() {
    ...
    if sudo podman ps -a | awk -v NAMES="my-httpd-container" '$NF == NAMES {found=1} END{if(found) exit 0; else exit 1}'; then
        echo "Container 'my-httpd-container' already exists. Skipping container creation."
    else
        # Check if the volume already exists
        if sudo podman volume ls | awk -v VOLUME="httpd" '$2 == VOLUME {found=1} END{if(found) exit 0; else exit 1}'; then
            echo "Volume 'httpd' already exists. Skipping volume creation."
        else
